...
all: templates indexes bundle lint
...
templates:
	docker run --rm --user $(USER_ID):$(GROUP_ID) \
		...
		-v ./tool:/tool \
		python:3.11-slim \
		bash -c "pip install --no-cache-dir --user -r /tool/requirements.txt && PATH=/tmp/.local/bin:\$$PATH python /tool/generate_templates.py"
